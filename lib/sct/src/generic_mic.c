...
#if FN_SUP
const unsigned char ev_controllable[4] = {1, 0, 1, 0};
...
const unsigned char sup_data[34] = {2, EV_btnStop, 0, 0, EV_btnMove, 0, 1, 3, EV_move, 0, 2, EV_btnStop, 0, 0, EV_btnMove, 0, 1, 2, EV_btnStop, 0, 3, EV_btnMove, 0, 2, 3, EV_btnStop, 0, 3, EV_stop, 0, 0, EV_btnMove, 0, 2};
...
void SCT_run_step()
{
    ...
    { // clear buffer, executing all no controllable events (NCE)
        make_transition(event);
        execCallback(event);
    }
    if (get_next_controllable(&event))
    { // find controllable event (CE)
