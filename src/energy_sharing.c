// Basic energy sharing code
// Does not utilise BLE connection to the DKB
// Transmits voltage readings over the serial monitor which is then recorded and live plotted to show
// voltage drop+ increase when energy is shared
...
static bool energy_sharing = false; // Flag to indicate if energy sharing is enabled
               ...
               uint16_t len)
{
    ...
    case 'g':
        LOG_INF("energy sharing enabled");
        energy_sharing = true;
        break;
    
    case 'i':
        LOG_INF("energy sharing disabled");
        energy_sharing = false;
        break;
    }
...
void ble_write_thread(void)
{
    ...
    for (;;)
    {
        ...
        if (energy_sharing){
            static const struct gpio_dt_spec v_b2b_en = true;
